...
| | |
|---|---|
| Commit | `07a572f` — "perf(reduce): start the model on the deterministic findings, not the seed (#7)", plus this session's subprocess-deadline and report-save fixes (`src/main.rs`, `tests/full_audit_mock.rs`) |
...
| Tests | ✅ 203 passed, 0 failed (162 unit tests in `src/**` + 41 black-box tests in `tests/*.rs`) |
