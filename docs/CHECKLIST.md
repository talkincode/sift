...
| | |
|---|---|
| Commit | `dd5bffa` — "fix: keep a chatty subprocess alive, and save the report byte-for-byte (#8)", plus this session's github-intake hardening (`src/main.rs`) |
...
| Tests | ✅ 208 passed, 0 failed (167 unit tests in `src/**` + 41 black-box tests in `tests/*.rs`) |
