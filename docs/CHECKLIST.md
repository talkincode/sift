...
| 5 | No unbounded blocking | ✅ Held | Model calls: `ureq` timeout in `model.rs`. Subprocesses: `run_command_with_timeout` (git fetch 120s, recursive local `sift` invocation 600s), which drains both pipes on reader threads so a chatty child is never mistaken for a hung one, and kills on expiry |
