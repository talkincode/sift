...
| | |
|---|---|
| Commit | `50e5c7d` — "fix(github): make the temp checkout unique, and guard the one recursive delete (#9)", plus this session's policy-cap fix (`src/report.rs`) |
| Date assessed | 2026-09-16 — build, tests, gate, the P0/P1 scan rows, and the P4a rows for Reduce (parallel batches, cost accounting, entry point), `doctor`, and the missing-key gate were re-verified this session; phase rows not touched by this session were audited at `f9a374b` and are carried forward unchanged |
| `cargo build` | ✅ pass |
| `make ci` (`fmt-check` + `test` + `clippy -D warnings` + `internal-gate`) | ✅ pass, exit 0 |
| Tests | ✅ 221 passed, 0 failed (176 unit tests in `src/**` + 45 black-box tests in `tests/*.rs`) |
...
| 8 | G | Invalid config fails loudly, never silently reverts to defaults | ✅ Done | Tests `dirty_values_reject_config_not_silent_default`, `valid_toml_wrong_types_reject_config_not_silent_default`, `rejects_dirty_env_lines` | `tests/policy_cli.rs` also covers the policy half end to end: an allowlist on disk suppresses a reviewed fixture and discloses it, a rule-only denylist cannot lift a fixture above its cap and is reported as `held ...`, an override tunes a production finding, and a matcher with neither `path` nor `rule` fails with exit 1 and names the missing criterion. That last case found that config errors dropped their cause: the loader reported only `invalid policy file <path>`. `main` now prints the whole error chain (`{e:#}`), so the message reads `invalid policy file <path>: policy key allowlist entries require path or rule`.
...
| 12 | F | Reduce opens on the local deterministic findings; the raw seed is never sent to a model | ✅ Done | `ReAct::run` runs `Skill::CoarseFilter` first and opens on the observation prompt (`run_opens_on_the_deterministic_observation_not_the_seed` asserts the first prompt is an `OBSERVATION:` and does not carry the seed; `tool_calls_still_resolve_the_seed_alias` keeps the `$SEED` protocol working). Measured with a mock endpoint recording every request body: requests per batch 2 → 1, payload 8,380,731 → 653,360 bytes on RAM-TA3 (11.6x less), 6,563,884 → 513,093 on TeamsACS, 271,778 → 24,504 on sift. The audit report discloses the entry point under `Diagnostics` |
...
| 5 | F | Project-local `sift-policy.toml` (`max_candidate_files`, `[[allowlist]]`, `[[denylist]]`, `[[severity_override]]`) | ✅ Done | `load_policy_config`/`parse_policy_config` in `config.rs`; test `parses_policy_schema_and_rejects_bad_severity`; `apply_policy`/`policy_match`/`policy_override_match` in `report.rs` The finding-side engine is now unit-tested end to end: `policy_allowlist_suppresses_matching_findings`, `policy_denylist_raises_severity_and_reports_the_reason`, `policy_denylist_stays_quiet_on_an_already_high_finding`, `policy_override_tunes_severity_in_both_directions`, `policy_allowlist_wins_over_denylist`, `policy_override_runs_last_and_wins_over_denylist`, `policy_path_matching_uses_a_normalized_substring`. Writing them found a real defect: policy was applied after the scope cap, so a rule-only denylist could lift a synthetic fixture to `high` while the ledger printed "test and fixture paths cannot exceed Low". The cap now runs after policy, a refused request is reported as `held ... (policy asked for high)`, and `policy_cannot_lift_a_fixture_above_its_scope_cap` plus `policy_cap_holds_in_the_gate_output` pin it. |
...
| # | Type | Item | Status | Evidence |
|---|---|------|--------|----------|
| 1 | F | `--benchmark` local telemetry (no model calls; optional USD cost estimate) | ✅ Done | `tests/benchmark_mode.rs` (3/3 passing). Input tokens come from `react::planned_prompts`: the observation prompts the Reduce stage actually sends, one per batch (RAM-TA3 planned 653,688 vs 653,360 sent). `tests/full_audit_mock.rs` pins it against a mock endpoint that records every request body: `planned_requests` equals the requests made, `planned_prompt_bytes` covers what was sent without overshooting by more than 10%, and the payload stays far below the seed |
| 2 | F | `sift github owner/repo` safe intake — never builds, installs, runs hooks, or touches submodules; inspects file/byte limits, `.gitmodules`, Git LFS before scanning | ✅ Done | `run_github_intake`, `parse_github_repo`, `inspect_checkout_dir`; tests `github_repo_parser_accepts_owner_repo_and_https`, `checkout_inspection_reports_lfs_and_limits`, `github_intake_rejects_non_github_url_without_network` (black-box). Both `git` fetch and the recursive local `sift` invocation run under `run_command_with_timeout` (120s / 600s hard deadlines with kill-on-timeout). That helper now drains both pipes while the child runs, because a child filling the pipe buffer blocks on write and would be killed at the deadline as if it had hung — the agent-gate child alone emits 686 KB on RAM-TA3. Tests `command_timeout_kills_a_hung_child`, `command_timeout_returns_output_of_a_finished_child`, `command_timeout_drains_output_larger_than_the_pipe_buffer` | Tests `temp_checkout_root_is_a_single_component_of_the_temp_dir`, `cleanup_checkout_removes_only_managed_temp_dirs`, `child_exit_codes_propagate`, `github_source_json_carries_every_field`, and `gate_and_benchmark_output_gain_the_github_source` cover the periphery: temp-checkout naming (unique within a clock tick, always one temp component), the recursive-delete guard, exit-code propagation (a signalled child is a failure, not success), and the `github_source` injection into gate/benchmark JSON including its error paths. Dot segments (`../..`) are now rejected at parse time instead of failing later inside git.
...
| 4 | F | `--save`/`--save-to` persisted reports (`reports/sift-audit-result-YYYYMMDD-NNN.md`) | ✅ Done | `save_audit_result`, `next_audit_result_path`, `utc_yyyymmdd`, `civil_from_days` in `main.rs`; test `audit_result_paths_number_within_a_date` covers numbering (other dates and unparsable names do not advance the counter, gaps are not reused, a missing directory still starts at 001). `full_audit_saves_the_report_it_prints` runs a mock full audit twice through `--save-to` and asserts the saved file is byte-identical to stdout and that the second run takes `-002.md`. The saved file now includes the trailing newline stdout had |
...
| 5 | No unbounded blocking | ✅ Held | Model calls: `ureq` timeout in `model.rs`. Subprocesses: all three spawn sites (git fetch 120s, recursive local `sift` invocation 600s, `eval-corpus` fixture 120s) now run under `run_command_with_timeout`, which drains both pipes on reader threads so a chatty child is never mistaken for a hung one, and kills on expiry. `sift eval-corpus` previously used a bare `Command::output()` with no deadline at all |
